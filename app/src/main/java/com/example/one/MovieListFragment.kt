...
class MovieListFragment: Fragment() , MovieAdapter.OnItemClickListener{
    val simpleAdapter = SimpleAdapter(this@MovieListFragment)
        ...
        call.enqueue(object : Callback<PopularMovies>{
            override fun onResponse(call: Call<PopularMovies>, response: Response<PopularMovies>) {
                if (response.isSuccessful){
                    rv_movie.adapter = MovieAdapter(response.body()!!.results, this@MovieListFragment)
                    rv_movie.apply {
    ...
    override fun onItemClicked(movie: Result) {
        requireView().findNavController().navigate(MovieListFragmentDirections.actionMovieListFragmentToSingleMovieFragment2(movie))
    }
